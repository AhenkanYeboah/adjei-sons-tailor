 ...
 * worker — lives in this one file. This is a genuine merge (built and
 * tested fresh from the original multi-file source), not a patch of
 * the earlier broken attempt.
 *
 * ── SETUP ──────────────────────────────────────────────────────────
 * 1. Load schema.sql, then config/seed_data.sql, into MySQL (see the
 *    original README — those two files are unchanged).
 * 2. Edit the CHANGE_ME_* placeholders directly below (DB, Paystack,
 *    Twilio credentials).
 * 3. Drop this single file anywhere your web server serves PHP from —
 *    no other files or folders are required for the site itself.
 *    (Twilio's SDK — needed only for the WhatsApp cron worker — still
 *    needs `composer require twilio/sdk` in the same folder as this
 *    file, creating a vendor/ directory next to it.)
 * 4. Visit the file directly in your browser — e.g.
 *    https://yoursite.com/adjei-sons-tailor.php — that's your homepage.
 *    Every other page is reached via ?page=xxx links generated by the
 *    app itself; you never need to type those by hand.
 *
 * ── RUNNING THE WHATSAPP WORKER ───────────────────────────────────
 * From the command line, in the same folder as this file:
 *   php adjei-sons-tailor.php send_whatsapp_notifications
 * Put that in cron to run every minute or so. It only runs from the
 * CLI — visiting it in a browser does nothing (by design, so a random
 * visitor can't trigger it).
 */

// Note: intentionally NOT using declare(strict_types=1) — PDO returns
// every column as a string by default, and this app passes those
// strings directly into typed function signatures like money(float
// $amount) throughout, relying on PHP's normal type coercion. That's
// the same behavior the original multi-file version had.

// These imports are safe even if the Twilio SDK isn't installed —
// `use` just creates an alias, it doesn't load or require the class to
// exist. The class is only actually needed inside cli_send_whatsapp(),
// which checks for vendor/autoload.php before touching it.
...
// --- Paystack ---
// PAYSTACK_PUBLIC_KEY is safe to expose in the browser — it only opens
// the payment popup. PAYSTACK_SECRET_KEY must NEVER be exposed to the
// browser, committed to a public repo, or pasted into chat/email —
// it's used server-side only, to confirm a payment actually succeeded.
// Get both from your Paystack dashboard under Settings → API Keys.
define('PAYSTACK_PUBLIC_KEY', 'pk_test_e7d2c45e0fbfe408a6a7cd79f633517490f6db90');
define('PAYSTACK_SECRET_KEY', 'sk_test_cedc917e734f3b0ab562f088c92f38ed3f76fd21'); // sk_test_...
define('PAYSTACK_CURRENCY', 'GHS');

// --- Twilio (WhatsApp) ---
// TWILIO_AUTH_TOKEN must never be exposed to the browser or committed
// publicly — anyone with it can send messages billed to your account.
// Get both from https://console.twilio.com.
...
define('TWILIO_CONTENT_SIDS', [
    'status_update'        => null, // e.g. 'HXb5b62575e6e4ff6129ad7c8efe1f983e' once approved by Meta
...
/**
 * Builds a link back to this same file with a ?page=... query string,
 * so the whole site works as a single file regardless of what it's
 * named or where it's placed. Extra query params (already safely
 * encoded by the caller) can be appended via $query, e.g.
 * url('configurator', 'fabric_id=3').
 */
...
// ============================================================
// LAYOUT — header/footer, CSS inlined (single-file build has no
// separate assets/ folder to serve a stylesheet from)
// ============================================================
...
// ============================================================
// WHATSAPP CRON WORKER (CLI only — see router at the bottom)
// ============================================================
...
if (PHP_SAPI === 'cli') {
    // CLI mode: only the WhatsApp worker runs from the command line.
    //   php adjei-sons-tailor.php send_whatsapp_notifications
    $cliCommand = $argv[1] ?? '';
